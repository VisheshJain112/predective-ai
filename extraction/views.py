    ...
    if request.method == "POST":
        ...
        for user in all_users:
            if user.username == username and user.password == password:
                ...
                list_of_keys = ['none','Chairman','Education','Parliament','Schools','speaker','sir']
                inx_needed = int(request.session['list_of_keys_inx'])
                if inx_needed < len(list_of_keys):

                    request.session['biased_key'].append(list_of_keys[inx_needed])
                    request.session['list_of_keys_inx'] = int(request.session['list_of_keys_inx']) + 1
                    biased_key = request.session['biased_key']
                    return render(request,'extraction.html',{'story' : story,'story_li' : story_li,'biased_key' : biased_key, 'user_dict' : pass_this_user,'extract' : "True"})
                else:
                    username = "admin"
                    password = "admin"
